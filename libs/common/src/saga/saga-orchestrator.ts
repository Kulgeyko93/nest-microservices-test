...
export class SagaOrchestrator<T> {
  ...
  public step(stepFn: (params: T) => Promise<void>): SagaOrchestrator<T> {
    const newStep = {
      run: stepFn,
    };
  ...
  public withCompensate(
    compensateFn: (params: T) => Promise<void>,
  ): SagaOrchestrator<T> {
    const lastStep = this.steps[this.steps.length - 1];
    if (!lastStep) {
      throw new BadRequestException(
        'Must call step() before calling withCompensate()',
      );
    }
    if (lastStep.compensate) {
      throw new BadRequestException(
        'One step cant have 2 compensate functions',
      );
    }
